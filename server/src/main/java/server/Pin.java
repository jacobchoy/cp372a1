 ...
 * A pin MAY be placed only at a coordinate that lies inside at least one
 * existing note.
 * A single pin coordinate MAY be associated with multiple notes if those notes
 * partially overlap.
 * 
 * Protocol identifies pins by coordinates (PIN x y, UNPIN x y); internal ID is
 * for tracking only.
 *
 * @author Jacob Choy
 * @version 1.0
...
public class Pin {
     ...
     * 
     * RFC Section 7.3: PIN x y - coordinates are used to identify pins in the
     * protocol.
     * 
     ...
     * 
     * Note: The protocol uses coordinates, not IDs, but IDs may be useful for
     * internal tracking.
     * 
